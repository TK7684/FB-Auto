...
# Configuration
MAX_CONVERSATIONS = 500  # Increased limit for production
...
async def scrape_conversations(limit: int = 500) -> List[Dict]:
    """Scrape conversations from inbox with pagination."""
    ...
    while fetched_count < limit:
        # Calculate how many to fetch in this batch (max 50 per call)
        batch_limit = min(50, limit - fetched_count)
        
        try:
            conversations_data = await fb.get_conversations(limit=batch_limit, after=after_cursor)
            threads = conversations_data.get("data", [])
            
            if not threads:
                break
                
            logger.info(f"Fetched {len(threads)} threads (Total: {fetched_count + len(threads)})")
            ...
            for thread in threads:
                ...
                # (We do this inside the loop to distribute rate limits)
            ...
            # Check for next page
            paging = conversations_data.get("paging", {})
...
async def main(dry_run: bool = False, limit: int = 50):
    """Main execution flow."""
    ...
    # 1. Scrape
    threads = await scrape_conversations(limit=limit)
    ...
    try:
        ...
        sheet_service.log_comment(
            post_id="SCRAPER",
            post_caption="Pattern Learning Run",
            comment_message=f"Scraped {limit} threads",
            reply_message=f"Ingested {count} patterns",
            comment_link="",
            status="Success"
        )
    except Exception as e:
        logger.error(f"Failed to log scrape run to Sheet: {e}")

if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser()
    parser.add_argument("--dry-run", action="store_true", help="Run without saving")
    parser.add_argument("--limit", type=int, default=50, help="Max conversations to scrape")
